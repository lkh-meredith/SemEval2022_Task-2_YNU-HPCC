...
if __name__=="__main__":
    ...
    set_seed(4)
    sentencemodel_save_path= '../model/SentenceTransformer/MySentTransformers/MySBERT_bert-base-multilingual-cased_'+str(train_batch_size)+'_'+str(num_epochs)
    ...
    results_file = os.path.join('..','submission','pretrain','MySBERT','dev.combined_results-'+ str( num_epochs ) +'_'+str(train_batch_size)+'.csv' )
    ...
    results_file = os.path.join( '..','submission','pretrain','MySBERT','eval.combined_results-'+ str(num_epochs) +'_'+str(train_batch_size)+'.csv' )
    # submission_data.to_csv(results_file,index=False)
    write_csv(submission_data,results_file)
