...
class TextCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, conv in enumerate(self.filter_list):
            h = torch.nn.functional.relu(conv(x)) # [bs, channel=1, seq-kernel_size+1, 1]
            mp = nn.MaxPool2d(
                kernel_size = (12 -self.kernel_list[i] +1, 1)
            )
            # mp: [bs, channel=3, w, h]
...
if __name__=="__main__":
    ...
    model_save_path=os.path.join('..', 'model', 'ZeroShot','BERT_textCNN')
    #两个编码器
    ...
    for epoch in range(num_epochs):
        ...
        results_file = os.path.join('..', 'submission', 'ZeroShot','BERT_textCNN','dev.combined_results-' + str(epoch+1) + '_' + str(train_batch_size) + '.csv')
    ...
    results_file = os.path.join('..', 'submission', 'ZeroShot','BERT_textCNN','eval.combined_results-' + str(best_epoch) + '_' + str(train_batch_size) + '.csv')
    write_csv(submission_data, results_file)
