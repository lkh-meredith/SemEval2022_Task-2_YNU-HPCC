...
if __name__=="__main__":
    ...
    model_save_path=os.path.join('..', 'model', 'ZeroShot','Prompt')

    train_samples, dev_samples, eval_samples = list(), list(), list()
    for index in range(len(train_data)):
        inp_example = InputExample(guid = index,text_a=train_data[index], label=train_label[index])
        train_samples.append(inp_example)
    for index in range(len(dev_data)):
        inp_example =InputExample(guid = index,text_a=dev_data[index], label=dev_label[index])
        dev_samples.append(inp_example)
    for index in range(len(eval_data)):
        inp_example =InputExample(guid = index,text_a=eval_data[index])
        eval_samples.append(inp_example)


    promptTemplate = ManualTemplate(
        text='{"placeholder":"text_a"} Question: Does it contain a MWE? {"mask"}.',
        tokenizer=tokenizer,
    )
    ...
    for epoch in range(num_epochs):
        ...
        results_file = os.path.join(model_save_path,
                                    'RESULTS_TABLE-dev_zeroshot-' + str(epoch + 1) + '_' + str(batch_size) + '.csv')
    ...
    submission_data = insert_to_submission_file(eval_submission_format_file, eval_path, eval_prediction_format_file,
                                                'one_shot')
    results_file = os.path.join('..', 'submission', 'ZeroShot', 'Prompt',
